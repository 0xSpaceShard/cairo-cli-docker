...
- An image is built for each version, with `TAG` being the version in the semver format (e.g. for version `0.7.1` the image is `shardlabs/cairo-cli:0.7.1`).
...
### Pull and check
```
$ docker pull shardlabs/cairo-cli:0.7.1

$ docker run shardlabs/cairo-cli:0.7.1 cairo-compile -v
cairo-compile 0.7.1

$ docker run shardlabs/cairo-cli:0.7.1 starknet -v
starknet 0.7.1
```
