...
- An image has been built for each cairo-lang version, with `TAG` being the version in the semver format (e.g. for version `0.9.0` the image is `shardlabs/cairo-cli:0.9.0`).
...
Since cairo-cli:0.9.0, images come with `openzeppelin-cairo-contracts` Python package preinstalled.
...
### Pull and check
```
$ docker pull shardlabs/cairo-cli:0.9.0

$ docker run shardlabs/cairo-cli:0.9.0 cairo-compile -v
cairo-compile 0.9.0

$ docker run shardlabs/cairo-cli:0.9.0 starknet -v
starknet 0.9.0
```
